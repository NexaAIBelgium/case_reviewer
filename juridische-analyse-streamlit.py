#!/usr/bin/env python3
"""
Juridische Analyse Streamlit App met Gemini 2.0 Flash en OCR
Een interactieve webapplicatie voor het analyseren van juridische documenten.

Installatie:
pip install streamlit google-generativeai python-dotenv PyPDF2 pdf2image pillow

Voor pdf2image moet je ook poppler installeren:
- Windows: Download van https://github.com/oschwartz10612/poppler-windows/releases/
...
# OCR Prompt
PROMPT_OCR = """Je bent een OCR specialist. Je taak is om de tekst uit de afbeelding(en) te extraheren en deze zo accuraat mogelijk te transcriberen. 

Belangrijke instructies:
- Behoud de originele structuur en formattering waar mogelijk
- Corrigeer duidelijke OCR-fouten (zoals l in plaats van I, 0 in plaats van O)
- Behoud paragraafindeling en kopjes
- Als je juridische termen of wetsartikelen ziet, zorg dan voor correcte spelling
...
def call_gemini(prompt, content, model_name="gemini-2.0-flash-exp"):
...
def extract_text_from_pdf(pdf_file):
    """Extract text uit PDF, gebruik OCR indien nodig"""
    text = ""
    ...
    try:
        ...
        st.warning(f"⚠️ Fout bij standaard PDF extractie: {e}")
    
    # Als er weinig of geen tekst is gevonden, gebruik OCR via Gemini
    if len(text.strip()) < 100:  # Minder dan 100 karakters betekent waarschijnlijk scan
        st.info("📸 PDF lijkt gescand, gebruik OCR via Gemini Vision...")
        ...
        try:
            ...
            for i, image in enumerate(images):
                ...
                st.text(f"Verwerken pagina {i + 1} van {len(images)}...")
                
                # Converteer PIL Image naar bytes
                img_byte_arr = io.BytesIO()
                image.save(img_byte_arr, format='PNG')
                img_byte_arr = img_byte_arr.getvalue()
                
                # Stuur naar Gemini Vision
            ...
            text = ocr_text
            st.success(f"✅ OCR voltooid voor {len(images)} pagina's")
            
        except Exception as e:
            st.error(f"❌ Fout bij OCR: {e}")
            st.info("💡 Tip: Zorg ervoor dat poppler geïnstalleerd is voor PDF conversie")
            return None
...
def main():
    ...
    st.markdown("**Analyseer juridische documenten met behulp van Gemini AI - Nu met OCR ondersteuning!**")
    ...
    with st.sidebar:
        ...
        st.markdown("""
        1. **Upload** je juridisch document (TXT of PDF)
        2. **Specificeer** het doel (optioneel)
        3. **Start** de analyse
        4. **Download** het rapport
        
        **Nieuw:** 📸 PDF's met scans worden automatisch verwerkt met OCR!
    ...
    with col1:
        ...
        uploaded_file = st.file_uploader(
            ...
            type=['txt', 'pdf'],
            help="Upload een tekstbestand of PDF met je juridische conclusie"
        )
    ...
    with col2:
        ...
        if uploaded_file:
            ...
            if uploaded_file.name.endswith('.pdf'):
                st.info("📸 PDF gedetecteerd - OCR beschikbaar indien nodig")
        else:
    ...
    if uploaded_file and st.session_state.api_key:
        if st.button("🚀 Start Analyse", type="primary", use_container_width=True):
            ...
            if not document_content:
                st.error("❌ Kon geen tekst uit het document extraheren")
                return
            
            # Toon preview van geëxtraheerde tekst
            with st.expander("👁️ Preview geëxtraheerde tekst (eerste 1000 karakters)"):
...
"""
                ...
                with st.spinner("Samenvatting maken..."):
                    output_5 = call_gemini(PROMPT_5_SUMMARY, output_4, model_name="gemini-2.0-flash-exp")
...
"""
                ...
                with st.spinner("Probleemanalyse..."):
                    problem_focus = call_gemini(PROMPT_7_PROBLEM_FOCUS, problem_content, model_name="gemini-2.0-flash-exp")
